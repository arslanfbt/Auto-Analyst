...
export default function CSVUploadDialog({
  ...
  // Auto-trigger description generation ONLY on first open with preview data
  useEffect(() => {
    if (isOpen && filePreview && !description && !hasAutoGenerated) {
      ...
      // Auto-trigger description generation after a small delay (only first time)
    ...
    } else if (isOpen && filePreview && !hasAutoGenerated) {
      // Set basic info without auto-generating
      setDatasetName(filePreview.name || fileName.replace(/\.csv$/i, ''))
      setDescription(filePreview.description || `CSV data from ${fileName}`)
    }
