...
export default function CSVUploadDialog({
  ...
  useEffect(() => {
    if (isOpen && filePreview?.headers && !columnsInitialized) {
      // Check if we have cached columns for this dataset
      const datasetKey = `${fileName}_${filePreview.headers.join('_')}_${filePreview.rows.length}`
      const cachedColumns = getCachedColumns().get(datasetKey)
      
      if (cachedColumns && cachedColumns.every(col => filePreview.headers.includes(col))) {
        // Use cached column selection if valid
        console.log('🔧 Restoring cached column selection:', cachedColumns)
        setSelectedColumns(cachedColumns)
      } else {
        // Default: all columns selected (only once per open cycle)
        console.log('🔧 Initializing columns (no cache):', filePreview.headers)
  ...
  }, [isOpen, filePreview?.headers, columnsInitialized, fileName])
  ...
  const saveCachedDescription = (key: string, description: string, columns?: string[]) => {
    try {
      ...
      // Also save selected columns if provided
      if (columns) {
        const columnsCache = getCachedColumns()
        columnsCache.set(key, columns)
        const serializedColumns = Object.fromEntries(columnsCache)
        localStorage.setItem('csv_columns_cache', JSON.stringify(serializedColumns))
        console.log('💾 Saved description and columns to cache:', key, columns)
      } else {
  ...
  const getCachedColumns = (): Map<string, string[]> => {
    try {
      const cached = localStorage.getItem('csv_columns_cache')
      if (cached) {
        const parsed = JSON.parse(cached)
        return new Map(Object.entries(parsed))
      }
    } catch (error) {
      console.error('Error loading cached columns:', error)
    }
    return new Map()
  }
      ...
      if (filePreview.name === "Housing Dataset") {
        ...
        if (cachedDesc) {
          ...
          const timer = setTimeout(() => {
            // For initial generation, use all columns
            handleAutoGenerate(newDatasetName, datasetKey, null, filePreview.headers)
            setHasAutoGenerated(true)
          }, 100)
  ...
  const handleAutoGenerate = async (explicitDatasetName?: string, cacheKey?: string, customPreview?: any, columnsToSave?: string[]) => {
    ...
    try {
      ...
      if (response.data && response.data.description) {
        ...
        // Save to cache with columns if provided
        if (cacheKey) {
