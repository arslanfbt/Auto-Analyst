...
export default function CSVUploadDialog({
  ...
  useEffect(() => {
    if (isOpen && filePreview && !hasAutoGenerated) {
      ...
      // Check if we already have a generated description for this dataset
      const datasetKey = `${fileName}_${filePreview.headers.join('_')}` // Create unique key
      const existingGeneratedDesc = generatedDescriptions.get(datasetKey)
      
      if (existingGeneratedDesc) {
        // Use cached description
        console.log('📋 Using cached description for dataset:', newDatasetName)
        setDescription(existingGeneratedDesc)
        setHasAutoGenerated(true)
      } else {
        // Auto-trigger description generation after a small delay (EVERY time for new datasets)
        const timer = setTimeout(() => {
          handleAutoGenerate(newDatasetName) // Pass the dataset name explicitly
          setHasAutoGenerated(true) // Mark as auto-generated
  ...
  const handleAutoGenerate = async (explicitDatasetName?: string) => {
    if (!sessionId || !filePreview) {
      console.error('No session ID or file preview available for description generation')
      return
    }
    
    // Use explicit dataset name if provided, otherwise use state
    const currentDatasetName = explicitDatasetName || datasetName
    
    // Check if we already have a meaningful description (not just placeholder text)
    const isPlaceholderDescription = description === `CSV data from ${fileName}` || 
                                   description === 'Please describe what this dataset contains and its purpose' ||
                                   description === filePreview.description ||
                                   !description.trim()
    
    if (!isPlaceholderDescription) {
      // We already have a meaningful description, don't regenerate
      console.log('📋 Description already exists, not regenerating')
      return
    }
